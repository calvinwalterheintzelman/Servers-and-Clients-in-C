...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
	...
	if(numBytes > MAX_DATA_SIZE) {
		fprintf(stderr, "Process aborted due to potential buffer overflow\n");
		close(clntSockfd);
		exit(1);
	}
/*
 * The above if block fixes the buffer overflow vulnerability.
 * Now, if the client tries to send a string that is longer than
 * five bytes, the server client shuts down. This makes it 
 * impossible to create a buffer overflow under this server!
 */
